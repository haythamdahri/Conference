...
@WebServlet("/professor/deletemypaper")
public class MyPaperDeleteController extends HttpServlet {
...
private IPapier papierBusiness;
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			boolean is_deleted = papierBusiness.delete(Integer.parseInt(mypaper_id));
			if( is_deleted ) {
				session.setAttribute("message_mypaper_type", "success");
				session.setAttribute("message_mypaper", "Le papier dont id="+mypaper_id+" à été supprimé");
			}else {
				session.setAttribute("message_mypaper_type", "danger");
				session.setAttribute("message_mypaper", "Une erreur est survenue, veuillez ressayer!");
			}	
