...
@WebServlet("/profile/updateconference")
public class ConferenceUpdateController extends HttpServlet{
			...
			// Check that we have a file upload request
		      ...
		      upload.setSizeMax( maxFileSize );
		         ...
		         for( Entry<String, String> p : dict.entrySet() ) {
		        	 if( p.getKey().equals("conference_id") ) {
		        		 conference.setId(Integer.parseInt(p.getValue()));
		         ...
		         Conference old_conf = conferenceBusiness.find(conference.getId());
		         conference.setAdministrateur(old_conf.getAdministrateur());
		         boolean is_updated = conferenceBusiness.update(conference);
		        		 
		         if( is_updated ) {
					session.setAttribute("message_conference_type", "success");
					session.setAttribute("message_conference", "La conference dont id="+conference.getId()+" est mis à jour avec succé");
					response.sendRedirect(request.getContextPath()+"/profile?conference=all");
		         }else {
					session.setAttribute("message_conference_edit_type", "danger");
					session.setAttribute("message_conference_edit", "Une erreur est survenue, veuillez ressayer!");
					response.sendRedirect(request.getContextPath()+"/profile?conference="+conference.getId()+"&edit");
		         }
