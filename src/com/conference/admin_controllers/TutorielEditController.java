...
@WebServlet("/profile/updatetutoriel")
public class TutorielEditController extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
				...
				Tutoriel updated_tuto = new Tutoriel(tutoriel.getId(), titre, description, conference);
				boolean is_updated = tutorielBusiness.update(updated_tuto);
				
				if( is_updated ) {
					session.setAttribute("message_tutoriel_type", "success");
					session.setAttribute("message_tutoriel", "tutoriel est mis a jour avec succée");
					response.sendRedirect(request.getContextPath()+"/profile?tutoriel=all");
				}else {
					session.setAttribute("message_tutoriel_edit_type", "danger");
					session.setAttribute("message_tutoriel_edit", "Une erreur est survenue, veuillez ressayer!");
